...
def main() -> None:
    ...
    for item in watchlist:
        ...
        if ceremony_url:
            special_awards_auto = extract_special_awards_from_infobox(ceremony_url)

        guests_auto: list[str] = []
        guests_sources: list[str] = []

        # Fuentes de grilla / guías (solo premios/nominaciones, no talk shows)
        ...
        for src in sources:
            ...
            if st == "titantv_whatson":
                hhmm, presenters = scrape_titantv_whatson(url)
                if hhmm and not start_local:
                    start_local = hhmm
                if presenters:
                    guests_auto = merge_list_unique(guests_auto, presenters)
                    guests_sources = merge_list_unique(guests_sources, [url])

            if st == "tvpassport":
                hhmm, presenters = scrape_tvpassport(url)
                if hhmm and not start_local:
                    start_local = hhmm
                if presenters:
                    guests_auto = merge_list_unique(guests_auto, presenters)
        ...
        # Fallback: si no encontramos hora en guías, usar defaults del watchlist (si existían)
        if not start_local:
        ...
        auto_ev = build_auto_event_entry(
            item=item,
            d=next_date,
            top_nominated=top_nominated,
